...
func main() {
	...
	// * 2 workers
	// * do sum up when worker get 100 items
	// * do sum up every 50 milliseconds if worker get less than 100 items
	...
	agg := &aggregator.Aggregator[*Item, map[string]string, int]{
		...
		Action: func(sum map[string]string) (int, error) {
			...
			// and apply rate limit by involving redis client pool to avoid too many goroutines
			go func() {
				defer func() {
					redisClient <- 1
				}()
